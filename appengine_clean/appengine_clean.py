...
async def delete_old_versions_multiple(
    projects: List[str], num_to_keep: int, fake=False, sample_name=''
):
    await asyncio.wait(
        [
            delete_old_versions(project, num_to_keep=num_to_keep, fake=fake, sample_name=sample_name)
            for project in projects
        ]
    )
async def delete_old_versions(project: str, num_to_keep: int, fake=False, sample_name=''):
    """
    Delete old AppEngine versions.
    project: the GCP project ID
    num_to_keep: the number of versions of each AppEngine service to keep
    fake: if True, perform a dry run
    sample_name: If provided, only deletes version with names containing sample_name
    """
    ...
    for service, versions_for_service in versions.items():
        ...
        versions_for_service = [version for version in versions_for_service if sample_name in version[1]]
        ...
        for version in to_delete:
            if sample_name in version[1]:# only delete versions that look like the version name provided
...
async def async_main():
    ...
    parser.add_argument(
        '--sample_name', action='store', dest='sample_name', default='', help='If provided, only deletes version with names containing sample_name'
    )
    parser.add_argument(
    ...
    await delete_old_versions_multiple(
        args.project, args.num_to_keep, fake=args.dry_run, sample_name=args.sample_name
    )
def main():
